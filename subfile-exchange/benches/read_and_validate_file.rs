...
use subfile_exchange::{
    file_hasher::ChunkFile,
    file_reader::read_and_validate_file,
    publisher::{FileMetaInfo, SubfileManifest},
    types::{BlockRange, Subfile},
};

fn setup() -> (Subfile, ChunkFile) {
    ...
    let subfile = Subfile {
        ...
        chunk_files: [chunk_file.clone()].to_vec(),
    };
...
fn read_and_validate_file_benchmark(c: &mut Criterion) {
    let (subfile, chunk_file) = black_box(setup());

    c.bench_function("read_and_validate_file", |b| {
        b.iter(|| read_and_validate_file(&subfile, &chunk_file))
    });
}
