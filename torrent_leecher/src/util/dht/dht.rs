use crate::util::dht::DHT_BOOTSTRAP;
...
use crate::util::{
    buffers::ByteString,
    librqbit_core::{id20::Id20, peer_id::generate_peer_id},
};
...
pub struct DhtStats {
    #[serde(serialize_with = "crate::util::dht::utils::serialize_id20")]
    pub id: Id20,
...
impl Dht {
    ...
    pub async fn with_config(config: DhtConfig) -> anyhow::Result<Self> {
        ...
        let bootstrap_addrs = config
            .bootstrap_addrs
            .unwrap_or_else(|| DHT_BOOTSTRAP.iter().map(|v| v.to_string()).collect());
