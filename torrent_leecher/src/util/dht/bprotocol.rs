...
// use crate::lib::rqbit_bencode::ByteBuf;
...
pub fn serialize_message<'a, W: Write, BufT: Serialize + From<&'a [u8]>>(
    ...
    match kind {
        MessageKind::Error(e) => {
            ...
            Ok(rqbit_bencode::bencode_serialize_to_writer(msg, writer)?)
        }
...
{
    let de: RawMessage<ByteBuf> = rqbit_bencode::from_bytes(buf)?;
    match de.message_type {
        MessageType::Request => match (&de.arguments, &de.method_name, &de.response, &de.error) {
            (Some(_), Some(method_name), None, None) => match method_name.as_ref() {
                b"find_node" => {
                    let de: RawMessage<BufT, FindNodeRequest> = rqbit_bencode::from_bytes(buf)?;
                    Ok(Message {
                ...
                b"get_peers" => {
                    let de: RawMessage<BufT, GetPeersRequest> = rqbit_bencode::from_bytes(buf)?;
                    Ok(Message {
                        transaction_id: de.transaction_id,
                        version: de.version,
                        ip: de.ip.map(|c| c.addr),
                ...
                b"ping" => {
                    let de: RawMessage<BufT, PingRequest> = rqbit_bencode::from_bytes(buf)?;
                    Ok(Message {
                        transaction_id: de.transaction_id,
                        version: de.version,
                        ip: de.ip.map(|c| c.addr),
        ...
        MessageType::Response => match (&de.arguments, &de.method_name, &de.response, &de.error) {
            ...
            (None, _, Some(_), None) => {
                let de: RawMessage<BufT, IgnoredAny, Response<BufT>> = rqbit_bencode::from_bytes(buf)?;
                Ok(Message {
                    transaction_id: de.transaction_id,
                    version: de.version,
                    ip: de.ip.map(|c| c.addr),
...
mod tests {
    ...
    use crate::util::dht::bprotocol::ByteBuf;
    use crate::util::{dht::bprotocol, rqbit_bencode};
    ...
    fn debug_hex_bencode(name: &str, data: &[u8]) {
        ...
        println!(
            "{:#?}",
            rqbit_bencode::dyn_from_bytes::<ByteBuf>(data.as_slice()).unwrap()
        );
    }
    ...
    fn test_deserialize_then_serialize(data: &[u8], name: &'static str) {
        dbg!(rqbit_bencode::dyn_from_bytes::<ByteBuf>(data).unwrap());
