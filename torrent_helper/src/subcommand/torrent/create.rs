...
Examples:
  ...
  pub input_positional: Option<InputTarget>,
  #[structopt(
    name = INPUT_FLAG,
    long = "input",
    short = "i",
  ...
  pub input_flag: Option<InputTarget>,
  #[structopt(
    long = "link",
    help = "Print created torrent `magnet:` URL to standard output"
  )]
  pub print_magnet_link: bool,
  #[structopt(
    long = "md5",
    short = "M",
...
impl Create {
  pub fn run(self, env: &mut Env, options: &Options) -> Result<(), Error> {
    let input = xor_args(
      "input_positional",
  ...
  pub fn generate_magnet_link(self, env: &mut Env, options: &Options) -> Result<MagnetLink, Error> {
    let input = xor_args(
      "input_positional",
      &self.input_positional,
      "input_flag",
    ...
    Ok(MagnetLink::from_metainfo_lossy(&metainfo)?)
  }
}
